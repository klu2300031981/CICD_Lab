...
class Requests extends Component {
    ...
    loadInventoryAndRequests = () => {
        callApi("GET", `${API_URL}/requests/inventory`, "", (invResponse) => {
    ...
    loadRequests = () => {
        callApi("GET", `${API_URL}/requests/readrequest`, "", (res) => {
            ...
            joblist = joblist.map(r => {
                ...
                if (r.status === 'pending' && available > 0) {
                    ...
                    callApi("PUT", `${API_URL}/requests/accept/${r.id}`, "", () => {});
                }
                return r;
            });
    ...
    saveRequest = () => {
        ...
        const url = this.state.id
            ? `${API_URL}/requests/update`
            : `${API_URL}/requests/insert`;
    ...
    updateRequest = (id) => {
        callApi("GET", `${API_URL}/requests/getrequest/` + id, "", (res) => {
            if (res.includes("401::")) { alert(res.split("::")[1]); return; }
    ...
    deleteRequest = (id) => {
        ...
        callApi("DELETE", `${API_URL}/requests/delete/` + id, "", this.afterSave);
    }
